...
export class S3Service {
  ...
  constructor() {
    ...
    if (!accessKeyId || !secretAccessKey) {
      throw new Error('AWS S3 credentials (AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY) are missing.')
    }
  ...
  async uploadFile(file: Express.Multer.File, folder = 'avatars'): Promise<string> {
    ...
    try {
      ...
      this.logger.error(`S3 upload failed for file ${file.originalname}: ${err.message}`)
