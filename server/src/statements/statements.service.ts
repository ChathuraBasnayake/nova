...
export class StatementsService {
  ...
  async generateStatementPdf(accountNumber: string, userId: number, res: Response): Promise<void> {
    ...
    doc.fillColor('#333333')
      .fontSize(10)
      .text(`Statement Generated: ${new Date().toLocaleDateString()}`, 400, 140, { align: 'right' })
    ...
    doc.fontSize(10)
      ...
      .text(`Current Balance: Rs. ${Number(account.balance).toLocaleString('en-US', { minimumFractionDigits: 2 })}`, 50, 235)
    ...
    if (txs.length === 0) {
      doc.text('No transaction history found for this account.', 50, y, { align: 'center' })
    } else {
      for (const t of txs) {
        ...
        if (y > 750) {
          ...
          doc.fontSize(9).font('Helvetica-Bold').fillColor('#666666')
            ...
            .text('Direction', 370, y)
            .text('Amount', 460, y, { align: 'right' })
          doc.moveTo(50, y + 15).lineTo(545, y + 15).stroke('#eee')
          y = y + 25
          doc.font('Helvetica').fillColor('#333333')
        }
        ...
        doc.fontSize(9)
          .text(dateStr, 50, y)
          .text(t.description || 'Bank Transfer', 130, y, { width: 140, height: 15, ellipsis: true })
          .text(t.category || 'Others', 280, y)
          .text(directionStr, 370, y)
        
        doc.fillColor(amountColor)
          .text(amountStr, 460, y, { align: 'right' })
